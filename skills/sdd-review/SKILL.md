...
4. **Gates, in parallel.** `model=$(sdd-review-model.sh $1)` resolves the reviewers' model from the constitution's `Review model` line (by issue type first, then by the triage size; `opus` when absent). Pass it as the `model` of every reviewer agent you launch. Default (`Review mode: paired`, or no such line in the constitution): launch the three paired reviewer agents, each with the pack path, issue, PR, commit sha and `rework_cycle: <cycle>`: `spec-test-reviewer` (gates spec-compliance + test-strategy), `design-quality-reviewer` (design-architecture + code-quality), `security-regression-reviewer` (security + regression). Each returns **two** YAML blocks; split them and `sdd-gate-result.sh post $pr <file>` for each, so the six gate results exist exactly as before. If the constitution's Verification section says `Review mode: single`, launch the six single-gate agents instead (`spec-reviewer`, `design-reviewer`, `test-reviewer`, `security-reviewer`, `regression-reviewer`, `quality-reviewer`), also with the pack path.
