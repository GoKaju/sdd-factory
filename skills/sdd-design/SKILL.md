...
4. **No self-review.** The design is written once, on the tier the repository's `.sdd/config.json` asks for (frontier by default for this phase, precisely so the first pass is the good one). Do not launch a reviewer agent on the document: the human judges it at Approval Gate 2 and the six Review Gates judge it again against the code. Report in the PR description what you left open for the human.
