...
- **max_rework_cycles:** 3
- **Review model:** opus — e.g. `sonnet for S, M · opus for L, Constitution`: which model runs the Review Gate agents, by triage size (S/M/L) or issue type (type wins). Absent → opus.
- **Review mode:** paired — `paired` (three reviewers, two gates each, over one shared review pack) or `single` (six independent reviewers). The six gate results are published either way.
