...
export const ledgerMarkdown = (rows: { phase: string; runs: number; minutes: number; usd: number }[], lang: 'es' | 'en'): string => {
  const t = lang === 'es'
    ? { title: 'Resumen SDD', phase: 'Fase', runs: 'Ejecuciones', min: 'Min', usd: 'USD', total: 'Total', by: 'generado por el worker sdd-factory' }
    : { title: 'SDD summary', phase: 'Phase', runs: 'Runs', min: 'Min', usd: 'USD', total: 'Total', by: 'generated by the sdd-factory worker' }
  const tot = rows.reduce((a, r) => ({ minutes: a.minutes + r.minutes, usd: a.usd + r.usd }), { minutes: 0, usd: 0 })
  const lines = rows.map((r) => `| ${r.phase} | ${r.runs} | ${r.minutes.toFixed(1)} | ${r.usd.toFixed(2)} |`)
  return [`### ${t.title}`, '', `| ${t.phase} | ${t.runs} | ${t.min} | ${t.usd} |`, '| --- | ---: | ---: | ---: |', ...lines,
