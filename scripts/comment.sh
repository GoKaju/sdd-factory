...
#
#   sdd comment find   <issue> <marker>           → prints the comment id, or nothing
#   sdd comment get    <issue> <marker>           → prints the comment body
#   sdd comment upsert <issue> <marker> <file|->  → creates or edits the comment with the file's body ('-' = stdin,
#                                                      so agents can pipe a heredoc without a Write tool)
#   sdd comment check  <issue> <marker> <T7>      → ticks the step whose line carries **T7**; refuses if an
