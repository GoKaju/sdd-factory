...
#
#   sdd rework pending <issue>   → one line per unapplied /rework comment: "<comment-id> <author>"
#   sdd rework apply   <issue>   → appends the steps to the Task, marks the comments applied (👀 by the
...
targets() { printf '%s\n' "$issue"; pr="$("$S/pr.sh" find "$issue" 2>/dev/null || true)"; [ -n "$pr" ] && printf '%s\n' "$pr"; }
...
case "$cmd" in
  pending) pending ;;
  apply)
    "$S/state.sh" require "$issue" final-review >/dev/null
    list="$(pending)"; [ -n "$list" ] || { echo "nothing pending"; exit 0; }
    task_id="$("$S/comment.sh" find "$issue" sdd:task)"; [ -n "$task_id" ] || die "no Task comment on #$issue; a /rework needs a Task to extend"
