...
#
#   sdd type get <issue>          → prints Feature | Change | Bug | Task | Constitution (empty if none)
#   sdd type set <issue> <type>   → assigns the type (the type must exist in the organization)
#   sdd type require <issue> <type>...
. "$(dirname "$0")/lib.sh"
