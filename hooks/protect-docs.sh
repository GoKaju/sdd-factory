...
#
# Flags (files under ~/.sdd/<owner>-<repo>/, written by the skills via `sdd flag`):
...
case "$rel" in
  docs/constitution.md)
    has_flag allow-constitution || block "docs/constitution.md changes only through a Constitution-type issue (/sdd-implement sets the flag). Rule W3."
    ;;
