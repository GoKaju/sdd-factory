...
export const waitingFor = (i: Pick<IssueSnapshot, 'state' | 'triageClean' | 'artifactClean' | 'reviewPassed'> & Partial<Pick<IssueSnapshot, 'reworkRequested'>>, autoApprove: ReadonlySet<string>): string | null => {
  switch (i.state) {
    ...
    case 'final-review': return i.reworkRequested || (autoApprove.has('Final') && i.reviewPassed) ? null : 'review and merge the PR, or /rework (Gate 4)'
    default: return null
  }
}
