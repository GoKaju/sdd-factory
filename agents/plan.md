...
1. **Context.** Read `docs/constitution.md`, the issue (`gh issue view N --comments`) and its triage comment (`sdd comment get N sdd:triage`) for the affected `docs/<domain>/<module>/`, the current `spec.md` and `design.md` if any, and the module's existing code (the Locations its design names, or its package) to reuse existing building blocks rather than invent parallel ones. The triage's **Clarifications** and **Assumptions** are the author's decisions: list them before writing and make every one visible in the spec (requirement, scenario, rejection row or domain concept). One you believe does not belong in the spec is explained in the PR description, never dropped; the completeness gate checks this.
