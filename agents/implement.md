...
1. **Context.** Read `docs/constitution.md` (Rules) and `docs/blueprint.md`, the Task comment (`sdd comment get N sdd:task`), and for Feature/Change the approved `spec.md` and `design.md`. `sdd ci list` shows the deterministic checks. **The worktree is fresh:** before the first test run, run the install command of `sdd ci list` and, when tests import other workspace packages, its build command, so tests resolve the workspace as CI does.
