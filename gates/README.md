...
| Gate | File | When |
| --- | --- | --- |
| `completeness` | `completeness.md` | before Approval Gate 1 (Plan), on `spec.md` |
...
- **Authority.** `docs/constitution.md` is binding; its `## Rules` are the checks. `docs/blueprint.md` is the convention reference: diverging from it is at most a WARNING. Where a gate item says "if the constitution has a rule about X", apply it only when such a rule exists, as written; where the constitution is silent, the item does not apply. Hierarchy: Constitution → Spec → Design → Task → Code. Code is never evidence that a document is wrong.
